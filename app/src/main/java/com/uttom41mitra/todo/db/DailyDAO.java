...
public class DailyDAO {
    ...
    public boolean AddDaily(ModelValue model){
        this.open();
        ...
        long id =   sqLiteDatabase.insert(DAILY_TABLE,null,values);
    ...
    public ArrayList<ModelValue> GetAllValue(){
        ...
        this.open();
        Cursor cursor = sqLiteDatabase.query(DAILY_TABLE,null,null,null,null,null,null);
    ...
    public boolean DeleteDaily(int ibd){
        this.open();
        int delete=  sqLiteDatabase.delete(DAILY_TABLE,COL_ID +"=?",new String[] {Integer.toString(ibd)} );
        return delete > 0;
    }
}
